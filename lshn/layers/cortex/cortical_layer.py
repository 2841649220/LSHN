...
class CorticalLayer(nn.Module):
    ...
    def step_fast(self, x_in: torch.Tensor, hyperedge_index: torch.Tensor,
        ...
        """
        # 1. 轴突延迟处理: 前突触脉冲经过延迟后再送入突触
        # AxonalDelayModule 处理 (max_edges,) 维度, 需要取 batch 平均
        pre_spk_for_delay = x_in.detach()
        if pre_spk_for_delay.dim() > 1:
            pre_spk_for_delay = pre_spk_for_delay.mean(dim=0)
        # 维度适配: 确保与 max_edges 匹配
        if pre_spk_for_delay.shape[0] > self.max_edges:
            pre_spk_for_delay = pre_spk_for_delay[:self.max_edges]
        elif pre_spk_for_delay.shape[0] < self.max_edges:
            pre_spk_for_delay = torch.nn.functional.pad(
                pre_spk_for_delay, (0, self.max_edges - pre_spk_for_delay.shape[0])
            )
        
        delayed_spk, stdp_delta = self.axonal_delay.step_fast(
            pre_spk_for_delay, self.prev_spk[:self.max_edges] 
            if self.prev_spk.shape[0] >= self.max_edges 
            else torch.nn.functional.pad(self.prev_spk, (0, self.max_edges - self.prev_spk.shape[0]))
        )
        ...
        # stdp_delta shape: (max_edges,) -> 截取/填充到 (num_neurons,)
        stdp_modulation = stdp_delta[:self.num_neurons] if stdp_delta.shape[0] >= self.num_neurons \
            else torch.nn.functional.pad(stdp_delta, (0, self.num_neurons - stdp_delta.shape[0]))
        if I_syn.dim() > 1:
        ...
        # 6. 更新脉冲计数
        self.spike_count += int(spk_out.detach().sum().item())

...
class DendriteCompartment(nn.Module):
    ...
    def forward(self, I_syn: torch.Tensor) -> torch.Tensor:
        ...
        if is_batched:
            ...
            # 2. 分支独立积分 - 需要为每个batch维护独立的状态
            # 扩展branch_potential以匹配batch维度
            if self.branch_potential.shape[0] != batch_size:
                branch_potential = self.branch_potential.unsqueeze(0).expand(batch_size, -1, -1)
            else:
            ...
            # 5. 分支重置 (Ca尖峰后重置该分支)
            branch_potential = branch_potential * (1.0 - above_threshold * 0.5)

            # 更新状态buffer (取batch平均, in-place 保护 buffer 注册)
            self.branch_potential.data.copy_(branch_potential.mean(dim=0))

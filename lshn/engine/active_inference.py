...
class ActiveInferenceEngine(nn.Module):
    ...
    def update_belief(self, observation: torch.Tensor, prediction_error: torch.Tensor):
        ...
        # 基于预测误差调整信念（渐进式更新）
        lr = 0.1
        self.belief_mean = self.belief_mean + lr * prediction_error
        # 方差基于误差大小适应性调整
        self.belief_logvar = self.belief_logvar + lr * (
            prediction_error.abs().log() - self.belief_logvar
        ).clamp(-5, 2)
    ...
    def compute_efe(self, current_state: torch.Tensor
        ...
        for pi_idx in range(self.num_policies):
            ...
            risk = F.kl_div(log_pref.unsqueeze(0).expand_as(pred_obs_log_prob),
                           pred_obs_prob, reduction='batchmean', log_target=False)
            ...
            # 近似: 当前后验熵 - 更新后预期后验熵
            current_entropy = 0.5 * self.belief_logvar.sum()
            # 假设每个策略减少的不确定性与观测精度成正比
            pred_precision = (-pred_obs_log_prob).exp().mean()
            expected_entropy_reduction = torch.log1p(pred_precision)

...
<p align="center">
  <strong>版本</strong>: v0.1.1 | <strong>作者</strong>: Apocalypse
</p>
...
# 初始化持续学习系统 (默认参数: 1000 皮层神经元 / 500 超边; 大规模场景
# 可扩展至 50 万神经元 + 5 万超边, 需 24G+ 显存)
model = LSHNModel(
    input_dim=128,
    hidden_dim=256,
    num_neurons=1000,        # 皮层神经元数
    num_groups=10,           # 功能分区（隐式 MoE）
    max_edges=500,           # 最大超边数
    initial_classes=2,
    device='cuda',
)
...
for x, target in dataloader:
    model.reset_sample_state()   # 样本级状态隔离 (每样本 20 步语义)
    for _ in range(20):
        output = model.forward_step(x, target)
    loss = criterion(output['output'], labels) + output.get('recon_loss', 0)
...
1. **跨任务遗忘**：当前共享皮层表征 + 全局误差信号的架构在合成数据 5 任务基准上**逐任务学习可达 ~100%**（v0.1.1 修复了 v0.1.0 中"任务 1 即崩溃 / 网络静默 / 预算单位错配"等问题），但旧任务在新任务训练后仍会显著遗忘。输出头冻结与 EWC-lite 结构重要性保护（`three_factor.importance_lambda`）已作为可选机制实现，实测稳定性-可塑性难以兼得，默认关闭。详细讨论见 [api_reference.md §7](api_reference.md) 与 [fix_plan_260806.md](fix_plan_260806.md)。
2. **主动推理未接线**：`enable_active_inference=True` 为预留接口（启用会告警），`ActiveInferenceEngine` 独立可用但未接入主流程。
3. **MODWT 为线性多尺度近似**：真实小波分解需要时间维输入，当前单帧特征管线使用"多尺度线性提取 + 泊松编码"。
...
<p align="center">
  <strong>最后更新</strong>: 2026 年 8 月 |
  <strong>版本</strong>: v0.1.1 |

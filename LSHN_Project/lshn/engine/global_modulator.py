...
class AstrocyteGate(nn.Module):
    ...
    def step_slow(self, mean_prediction_error: float, mean_firing_rate: float) -> torch.Tensor:
        ...
        new_calcium = decay * self.calcium + (1.0 - decay) * drive
        self.calcium.copy_(new_calcium)
...
class GlobalNeuromodulator(nn.Module):
    ...
    def step_slow(self, prediction_error: float, firing_rate: float,
        ...
        new_ach = decay_ach * self.ACh + (1.0 - decay_ach) * ach_drive
        self.ACh.copy_(new_ach.clamp(0.1, 10.0))
        ...
        new_ne = decay_ne * self.NE + (1.0 - decay_ne) * torch.sigmoid(ne_drive * 3.0)
        self.NE.copy_(new_ne.clamp(0.01, 2.0))
        ...
        new_da = decay_da * self.DA + (1.0 - decay_da) * torch.sigmoid(da_drive)
        self.DA.copy_(new_da.clamp(0.001, 1.0))
    ...
    def get_learning_rate_scale(self) -> torch.Tensor:
        ...
        修复: 对称化学习率缩放，确保 ACh=1.0 时 scale=1.0
        公式: scale = (ACh / ACh_baseline) * plasticity_gate
        """
        ach_baseline = 1.0  # ACh 中心值
        scale = (self.ACh / ach_baseline) * self.astrocyte.calcium

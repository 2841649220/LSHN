...
class FreeEnergyEngine:
    ...
    def __init__(self, kl_weight: float = 0.01, energy_lambda: float = 0.001):
        ...
        self.history = {
            "vfe_total": deque(maxlen=self.max_history),
            "J_total": deque(maxlen=self.max_history),
            "accuracy_loss": deque(maxlen=self.max_history),
            "complexity_loss": deque(maxlen=self.max_history),
            "energy_cost": deque(maxlen=self.max_history),
        }
    ...
    def compute_vfe(self, prediction_error: torch.Tensor, s_e_tensor: torch.Tensor, 
        ...
        s_e_clamped = s_e_tensor.clamp(min=1e-6, max=1.0 - 1e-6)
        structure_kl = torch.mean(
            s_e_clamped * torch.log(s_e_clamped / 0.5)
            + (1.0 - s_e_clamped) * torch.log((1.0 - s_e_clamped) / 0.5)
        ).item()
        # 2. 活跃度复杂度
    ...
    def get_decomposition_report(self) -> Dict[str, float]:
        ...
        # 修复: deque 不支持切片，使用 list 转换
        vfe_list = list(self.history["vfe_total"])
        J_list = list(self.history["J_total"])
        accuracy_list = list(self.history["accuracy_loss"])
        complexity_list = list(self.history["complexity_loss"])
        energy_list = list(self.history["energy_cost"])
        
        return {
            "vfe_recent_mean": sum(vfe_list[-recent_n:]) / recent_n,
            "J_recent_mean": sum(J_list[-recent_n:]) / recent_n,
            "accuracy_trend": sum(accuracy_list[-recent_n:]) / recent_n,
            "complexity_trend": sum(complexity_list[-recent_n:]) / recent_n,
            "energy_trend": sum(energy_list[-recent_n:]) / recent_n,

...
class DynamicExpansionHead(nn.Module):
    ...
    def expand(self, num_new_classes: int) -> List[nn.Parameter]:
        ...
        self.weight = nn.Parameter(torch.cat([old_weight, new_weight], dim=0))
        self.bias = nn.Parameter(torch.cat([old_bias, new_bias], dim=0))
        
        return [self.weight[old_classes:], self.bias[old_classes:]]

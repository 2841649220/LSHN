...
class LSHNModel(nn.Module):
    ...
    def forward_step(self, x: torch.Tensor, target: Optional[torch.Tensor] = None
        ...
        with torch.autocast(device_type=device_type,
            ...
            # 2. 海马体快速编码
            spk_hippo = self.hippocampus.step_fast(spk_encoded)  # (batch, hidden_dim) or (hidden_dim,)
            if spk_hippo.dim() == 1:
                spk_hippo = spk_hippo.unsqueeze(0)
    ...
    def _on_slow_clock(self, target: Optional[torch.Tensor] = None):
        ...
        # 修复: 改进 active_neurons_ratio 计算，考虑加权平均
        neuron_mask = self.cortex.prune_growth.neuron_mask.float()
        edge_mask = self.cortex.prune_growth.edge_mask.float()
        active_neurons_ratio = float((neuron_mask * self.cortex.cell.get_firing_rate().clamp(0, 1)).sum() / 
                                      max(1.0, neuron_mask.sum().item()))
        
        vfe_dict = self.vfe_engine.compute_vfe(
            prediction_error=prediction_error,
            s_e_tensor=self.cortex.synapse.s_e.detach(),
            active_neurons_ratio=active_neurons_ratio,
            synaptic_events=spike_count,
            precision=self._last_modulation.get("ACh", torch.tensor(1.0)).item() if self._last_modulation else 1.0
        )
        ...
        # 修复: 传递 inh_adj 到 step_slow，实现 MoE 抑制联动
        homeo_result = self.cortex.step_slow(global_e, M_global, R_replay, T_temp, 
                                              inh_adj=budget_result.get("inh_adj", 0.0))
        ...
        # 修复: 重置脉冲计数窗口 (滑动窗口模式)
        self.cortex.reset_spike_count_window()
    ...
    def _on_ultra_slow_clock(self):
        ...
        try:
            ...
            if cold_indices.numel() > 0:
                ...
                # 4. 修复: 改进冷槽重置逻辑，引入冷却期概念
                with torch.no_grad():
                    self.cortex.synapse.w_hat.data[cold_indices] = 0.0
                    self.cortex.synapse.s_e.data[cold_indices] = 0.5
                    # 重置资格迹，避免历史活动影响新生
                    self.cortex.synapse.e_trace.data[cold_indices] = 0.0
                    self.cortex.synapse.pre_trace.data[cold_indices] = 0.0
                    self.cortex.synapse.post_trace.data[cold_indices] = 0.0
    ...
    def reset(self):
        ...
        if hasattr(self.cortex, 'reset'):
            self.cortex.reset()
        if hasattr(self.hippocampus, 'reset'):
            self.hippocampus.reset()
        if hasattr(self.replay_generator, 'reset'):
            self.replay_generator.reset()
        if hasattr(self.neuromodulator, 'reset'):
            self.neuromodulator.reset()
        if hasattr(self.vfe_engine, 'reset'):
            self.vfe_engine.reset()
        if hasattr(self.budget_ctrl, 'reset'):
        ...
        self._step_spike_count = 0
        # 修复: 重置皮层脉冲计数窗口
        if hasattr(self.cortex, 'reset_spike_count_window'):
            self.cortex.reset_spike_count_window()

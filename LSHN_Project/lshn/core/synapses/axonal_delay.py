...
class AxonalDelayModule(nn.Module):
    ...
    def step_fast(self, pre_spk: torch.Tensor, post_spk: torch.Tensor
        ...
        # 5. 前进指针 (使用 in-place 操作保护 buffer 状态)
        with torch.no_grad():
    ...
    def get_delay_stats(self) -> dict:
        ...
        # 使用稳定的 softmax 实现，添加 eps 防止数值问题
        d_scaled = d - d.max()  # 数值稳定性
        probs = torch.softmax(d_scaled, dim=0)
        log_probs = torch.log_softmax(d_scaled, dim=0)

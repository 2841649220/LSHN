...
class SynapticScaling(nn.Module):
    ...
    def update_rates(self, spk: torch.Tensor):
        ...
        Args:
            spk: (num_neurons,) 或 (batch, num_neurons) 当前步的脉冲 {0, 1}
        """
        # 支持 batch 输入
        if spk.dim() > 1:
            spk = spk.float().mean(dim=0)
        else:
            spk = spk.float()
            
        with torch.no_grad():
            self.firing_rate_ema.data.mul_(self.ema_decay).add_(
                (1.0 - self.ema_decay) * spk
            )
    ...
    def compute_scaling_factors(self) -> torch.Tensor:
        ...
        """
        # 防止除零 - 使用更小的下限
        rate = self.firing_rate_ema.clamp(min=1e-8)
    ...
    def apply_scaling(self, w_hat: torch.Tensor, 
...
class IntrinsicExcitabilityPlasticity(nn.Module):
    ...
    def __init__(self, num_neurons: int, target_rate: float = 0.05,
        ...
        if dynamic_range:
            # 根据网络规模动态调整范围
            # 公式: range ∝ sqrt(num_neurons) 以适应不同规模网络
            scale_factor = max(1.0, (num_neurons / 1000.0) ** 0.5)
            self.theta_min = -1.0 * scale_factor
            self.theta_max = 1.0 * scale_factor
        else:
    ...
    def step_slow(self, firing_rate_ema: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            # 限制调整幅度 - 使用可配置的对称范围
            self.theta_ie.data.clamp_(self.theta_min, self.theta_max)
...
class HomeostaticController(nn.Module):
    ...
    def __init__(self, 
        ...
        self.ie_plasticity = IntrinsicExcitabilityPlasticity(
            num_neurons, 
            target_rate=target_rate,
            ie_learning_rate=ie_learning_rate,
            theta_min=theta_min,
            theta_max=theta_max,
            **factory_kwargs
        )

...
| 机制 | 技术实现 | 生物学对应 |
|:---:|:---|:---|
| **多时间尺度解耦** | $\mathbb{T}_{\text{fast}}$ (1 ms) / $\mathbb{T}_{\text{slow}}$ (100 ms) / $\mathbb{T}_{\text{ultra}}$ (1000 ms) | 离子通道 vs G 蛋白偶联受体 vs 结构可塑性 |
...
| **脉冲预算控制** | PI 控制器动态调节阈值/抑制强度/能量正则 | 目标能耗 + MoE 抑制联动 |
| **动态输出扩容** | 增量分类任务自动增加输出神经元 | 无需重训练 |
| **维度对齐工具** | 自动处理张量维度不匹配问题 | 鲁棒性↑ |
| **滑动窗口统计** | 脉冲计数使用 100 步滑动窗口 | 长期统计准确性↑ |
...
| 引擎 | 职责 | 关键功能 |
|:---:|:---|:---|
| `ClockSyncEngine` | 多时间尺度时钟同步 | 1 ms/100 ms/1000 ms 事件触发 |
...
homeostatic:
  target_rate: 0.05
  dynamic_range: true      # 根据网络规模动态调整阈值范围
```
